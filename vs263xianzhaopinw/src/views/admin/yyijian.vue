  ...
  <section style="margin: 10px">
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px; margin-left: 10px" >
      <el-form :inline="true" :model="filters">
        <el-form-item>
          <el-input v-model="filters.yyijianName" size="small" placeholder="名称" ></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="info" size="small" @click="query">查询</el-button>
        </el-form-item>
        <el-form-item v-if="role == 'yonghu'">
          <el-button type="primary" size="small" @click="handleAdd">新增</el-button>
        </el-form-item>
        <el-form-item>
          <el-button type="danger" size="small" @click="deleteYyijians" :disabled="selectedIds.length === 0">批量删除</el-button>
        </el-form-item>
    ...
    <el-table ref="exportTableRef" @selection-change="handleSelectionChange" id="myTable" :data="datalist" border stripe v-loading="listLoading" highlight-current-row max-height="800" size="small" style="width: 100%">
      ...
      <el-table-column sortable prop="yyijianId" align="center" label="Id" min-width="20%" v-if="false"></el-table-column>
      <el-table-column sortable prop="yyijianName" align="center" label="名称" min-width="40%"></el-table-column>
      <el-table-column prop="yyijianMark" align="center" label="详情" :show-overflow-tooltip="true" min-width="80%"></el-table-column>
      <el-table-column prop="yyijianMark1" align="center" label="回复" :show-overflow-tooltip="true" min-width="80%"></el-table-column>
      <el-table-column prop="yonghuId" align="center" :label=jcpeizhi.yonghuBieming min-width="20%" v-if="false"></el-table-column>
      <el-table-column prop="yonghuName" align="center" :label=jcpeizhi.yonghuBieming min-width="40%" v-if="role == 'admin'"></el-table-column>
      <el-table-column sortable prop="yyijianDate" align="center" label="时间" width="130" :formatter="formatDate" v-if="false"></el-table-column>
      <!--操作按钮开始-->
      <el-table-column label="操作" align="center" width="150">
        <template #default="scope">
          <el-button type="primary" size="mini" @click="handleEdit(scope.$index, scope.row)" v-if="role == 'yonghu'">编辑</el-button>
    ...
    <!--表单-->
    <el-dialog :title=jcpeizhi.yyijianBieming v-model="formVisible" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" :rules="rules" label-width="100px">
        <el-form-item label="名称" prop="yyijianName">
          <el-input v-model="formData.yyijianName" style="width: 50%"></el-input>
        </el-form-item>
        <el-form-item label="详情" prop="yyijianMark">
          <el-input type="textarea" :rows="4" v-model="formData.yyijianMark" style="width: 80%"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog title="上传信息" v-model="formShangchuan" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="附件" prop="yyijianImg">
          <el-input v-model="formData.yyijianImg" readonly="true" style="width: 50%"></el-input>
    ...
    <el-dialog title="处理信息" v-model="formChuli" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="回复" prop="yyijianMark1">
          <el-input type="textarea" :rows="4" v-model="formData.yyijianMark1" style="width: 80%"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
    ...
    <el-dialog title="审核信息" v-model="formShenhe" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="状态" prop="yyijianType">
          <el-select v-model="formData.yyijianType" style="width: 50%" placeholder="请选择">
            <el-option label="通过" value="1"></el-option>
            <el-option label="拒绝" value="2"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="审核" prop="yyijianMark3">
          <el-input type="textarea" :rows="4" v-model="formData.yyijianMark3" style="width: 80%"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
    ...
    <el-dialog title="导入信息" v-model="formDaoru" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="excel表" prop="yyijianImg">
          <el-input v-model="formData.yyijianImg" readonly="true" style="width: 50%"></el-input>
          <el-button type="primary" size="small" style="margin-left: 15px" @click="showUpload">上传</el-button>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      selectedIds: [],//多选数据
      rules: {     
        yyijianName: [{ required: true, message: "不能为空", trigger: "blur" }],
        yyijianMark: [{ required: true, message: "不能为空", trigger: "blur" }],
        yyijianMark1: [{ required: true, message: "不能为空", trigger: "blur" }],
        yyijianMark2: [{ required: true, message: "不能为空", trigger: "blur" }],
        yyijianDouble: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        yyijianDouble1: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        yyijianDouble2: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        yyijianZong: [{ required: true, validator: isIntNum, trigger: "blur" }],
        yyijianZong1: [{ required: true, validator: isIntNum, trigger: "blur" }],
        yyijianZong2: [{ required: true, validator: isIntNum, trigger: "blur" }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    //富文本
    editorChange(info) {
      this.formData.yyijianMark = info;
    },
    //打开修改界面
    ...
    handleDelete(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          ...
          let para = {
            delIds: row.yyijianId,
          };
          let url = base + "/deleteYyijian";
    ...
    */
    ...
    handleSelectionChange(selection) {
      this.selectedIds = selection.map((item) => item.yyijianId);
    ...
    getDatas() {
      ...
      let para = {
        yyijianName: this.filters.yyijianName,
        yxtypeId: this.filters.yxtypeId,
      ...
      //console.log(para)
      this.listLoading = true;
      let url = base + "/getYyijians?page=" + this.page.currentPage;
      //console.log(url);
      request.post(url, para).then((res) => {
        console.log(res);
    ...
    getYxtypeList() {
      let para = {};
      this.listLoading = true;
      let url = base + "/yxtypeComboList";
      request.post(url, para).then((res) => {
        if (res.code == 200) {
          this.yxtypeList = res.resdata;
        }
      });
    },
    //获取buzhi
    ...
    shangchuanSave() {
      this.$refs["formDataRef"].validate((valid) => {
        if (valid) {
          let url = base + "/addYyijian?yyijianId=" + this.formData.yyijianId;
          this.btnLoading = true;
          request.post(url, this.formData).then((res) => {
            if (res.code == 200) {
              this.btnLoading = false;
    ...
    //添加修改
    save() {
      if (this.formData.yyijianId != null) {
        //如果有ID就修改数据
        this.$refs["formDataRef"].validate((valid) => {
          if (valid) {
            let url = base + "/addYyijian?yyijianId=" + this.formData.yyijianId;
    ...
    danduShenhe(index, row) {
      this.formData = JSON.parse(JSON.stringify(row));
      if(this.formData.yyijianType==0){
    ...
    //根据数据状态显示文本
    zhuangtaiType(row){
        if(row.yyijianType==0){
             return "启用";
        }
        if(row.yyijianType==1){
             return "禁用";
        }
    },
    //单项更改状态
    shezhiZhuangtai(index, row,zhuangtai) {
      this.listLoading = true;
      let para = {
        yyijianType: zhuangtai,
        yyijianId: row.yyijianId,
      };
