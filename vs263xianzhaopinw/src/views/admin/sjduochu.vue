  ...
  <section style="margin: 10px">
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px; margin-left: 10px" >
      <el-form :inline="true" :model="filters">
        <el-form-item>
          <el-input v-model="filters.sjduochuName" size="small" placeholder="名称" ></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="info" size="small" @click="query">查询</el-button>
        </el-form-item>
        <el-form-item>
          <el-button v-if="role == 'user'" type="primary" size="small" @click="handleAdd">新增</el-button>
        </el-form-item>
        <el-form-item>
          <el-button type="danger" size="small" @click="deleteSjduochus" :disabled="selectedIds.length === 0">批量删除</el-button>
        </el-form-item>
    ...
    <el-table ref="exportTableRef" @selection-change="handleSelectionChange" id="myTable" :data="datalist" border stripe v-loading="listLoading" highlight-current-row max-height="800" size="small" style="width: 100%">
      ...
      <el-table-column sortable prop="sjduochuId" align="center" label="Id" min-width="20%" v-if="false"></el-table-column>
      ...
      <el-table-column prop="sjxingtaiId" align="center" :label=jcpeizhi.sjxingtaiBieming min-width="20%" v-if="false"></el-table-column>
      <el-table-column prop="sjxingtaiName" align="center" :label=jcpeizhi.sjxingtaiBieming min-width="40%"></el-table-column>
      <el-table-column prop="sjduochuName" align="center" label="名称" min-width="40%"></el-table-column>
      <el-table-column prop="sjduochuZong" align="center" label="点赞" width="50" ></el-table-column>
      <el-table-column prop="sjduochuType" align="center" label="状态" width="50" :formatter="zhuangtaiType" ></el-table-column>
      <!--显示图片开始-->
      <el-table-column prop="sjduochuImg" align="center" label="图片" width="130">
        <template #default="scope">
          <div style="width: 110px;height: 110px;display: flex;align-items: center;">
            <el-image fit="fill" :src="scope.row.sjduochuImg" :preview-src-list="[scope.row.sjduochuImg]"></el-image>
          </div>
        </template>
      </el-table-column>
      <!--显示图片结束-->
      <el-table-column sortable prop="sjduochuDate" align="center" label="时间" width="130" :formatter="formatDate" v-if="false"></el-table-column>
      <!--操作按钮开始-->
      <el-table-column label="操作" align="center" width="150">
        <template #default="scope">
          <el-button v-if="scope.row.sjduochuType == 0 && role == 'admin'" type="success" size="mini" @click="shezhiZhuangtai(scope.$index, scope.row,1)" >置顶</el-button>
          <el-button v-if="scope.row.sjduochuType == 1 && role == 'admin'" type="success" size="mini" @click="shezhiZhuangtai(scope.$index, scope.row,0)" >取消</el-button>
          <el-button v-if="role == 'user'" type="success" size="mini" @click="handleEdit(scope.$index, scope.row)" >编辑</el-button>
    ...
    <!--表单-->
    <el-dialog :title=jcpeizhi.sjduochuBieming v-model="formVisible" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" :rules="rules" label-width="100px">
        <el-form-item label="名称" prop="sjduochuName">
          <el-input v-model="formData.sjduochuName" style="width: 50%"></el-input>
        </el-form-item>
        <el-form-item label="图片" prop="sjduochuImg">
          <el-input v-model="formData.sjduochuImg" readonly="true" style="width: 50%"></el-input>
          <el-button type="primary" size="small" style="margin-left: 15px" @click="showUpload">上传</el-button>
        </el-form-item>
        <el-form-item :label=jcpeizhi.sjxingtaiBieming prop="sjxingtaiId">
          <el-select v-model="formData.sjxingtaiId" placeholder="请选择" style="width: 50%">
            <el-option label="请选择" value="0"> </el-option>
            <el-option v-for="item in sjxingtaiList" :key="item.sjxingtaiId" :label="item.sjxingtaiName" :value="item.sjxingtaiId"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
          <WangEditor v-model="formData.sjduochuMark" :isClear="isClear" @change="editorChange"></WangEditor>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog title="上传信息" v-model="formShangchuan" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="附件" prop="sjduochuImg">
          <el-input v-model="formData.sjduochuImg" readonly="true" style="width: 50%"></el-input>
          <el-button type="primary" size="small" style="margin-left: 15px" @click="showUpload">上传</el-button>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="处理信息" v-model="formChuli" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        ...
        <el-form-item label="简介" prop="sjduochuMark1">
          <el-input type="textarea" :rows="4" v-model="formData.sjduochuMark1" style="width: 80%"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
    ...
    <el-dialog title="审核信息" v-model="formShenhe" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="状态" prop="sjduochuType">
          <el-select v-model="formData.sjduochuType" style="width: 50%" placeholder="请选择">
            <el-option label="通过" value="1"></el-option>
            <el-option label="拒绝" value="2"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="审核" prop="sjduochuMark3">
          <el-input type="textarea" :rows="4" v-model="formData.sjduochuMark3" style="width: 80%"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
    ...
    <el-dialog title="导入信息" v-model="formDaoru" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="excel表" prop="sjduochuImg">
          <el-input v-model="formData.sjduochuImg" readonly="true" style="width: 50%"></el-input>
          <el-button type="primary" size="small" style="margin-left: 15px" @click="showUpload">上传</el-button>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      selectedIds: [],//多选数据
      rules: {     
        sjduochuName: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjduochuMark: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjduochuMark1: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjduochuMark2: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjduochuMark3: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjduochuDouble: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        sjduochuDouble1: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        sjduochuDouble2: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        sjduochuZong: [{ required: true, validator: isIntNum, trigger: "blur" }],
        sjduochuZong1: [{ required: true, validator: isIntNum, trigger: "blur" }],
        sjduochuZong2: [{ required: true, validator: isIntNum, trigger: "blur" }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    //富文本
    editorChange(info) {
      this.formData.sjduochuMark = info;
    },
    //打开修改界面
    ...
    handleDelete(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          ...
          let para = {
            delIds: row.sjduochuId,
          };
          let url = base + "/deleteSjduochu";
    ...
    handleConfirm() {
      ...
      request.post(url, formData).then((res) => {
        ...
        console.log(furl + "  ------------");
        this.formData.sjduochuImg = furl;
        this.hideUpload();
        console.log(res);
      });
    },
    ...
    */
    ...
    handleSelectionChange(selection) {
      this.selectedIds = selection.map((item) => item.sjduochuId);
    ...
    getDatas() {
      ...
      let para = {
        sjduochuName: this.filters.sjduochuName,
      ...
      //console.log(para)
      this.listLoading = true;
      let url = base + "/getSjduochus?page=" + this.page.currentPage;
      //console.log(url);
      request.post(url, para).then((res) => {
        console.log(res);
    ...
    shangchuanSave() {
      this.$refs["formDataRef"].validate((valid) => {
        if (valid) {
          let url = base + "/addSjduochu?sjduochuId=" + this.formData.sjduochuId;
          this.btnLoading = true;
          request.post(url, this.formData).then((res) => {
            if (res.code == 200) {
              this.btnLoading = false;
    ...
    //添加修改
    save() {
      if (this.formData.sjduochuId != null) {
        //如果有ID就修改数据
        this.$refs["formDataRef"].validate((valid) => {
          if (valid) {
            let url = base + "/addSjduochu?sjduochuId=" + this.formData.sjduochuId;
    ...
    danduChuli(index, row) {
      this.formData = JSON.parse(JSON.stringify(row));
      if(this.formData.sjduochuType==1){
        this.formChuli = true;
      }else{
        this.$message({
          message: "不允许修改",
    ...
    //根据数据状态显示文本
    zhuangtaiType(row){
        if(row.sjduochuType==0){
             return "普通";
        }
        if(row.sjduochuType==1){
             return "置顶";
        }
    },
    //根据数据状态显示文本
    ...
    shezhiZhuangtai(index, row,zhuangtai) {
      this.listLoading = true;
      let para = {
        sjduochuType: zhuangtai,
        sjduochuId: row.sjduochuId,
      };
