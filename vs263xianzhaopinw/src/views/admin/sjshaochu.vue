  ...
  methods: {
    ...
    */
    exportTable() {
      ...
      const selectedData = this.datalist.filter(item => this.selectedIds.includes(item.sjshaochuId));

      // 3. 提取当前模块需要的字段并转换为中文表头
      const exportJson = selectedData.map((item, index) => {
        let obj = {};
        obj["序号"] = index + 1;
        obj[this.jcpeizhi.yonghuBieming || "企业"] = item.yonghuName || "";
        obj[this.jcpeizhi.shujuBieming || "职位"] = item.shujuName || "";
        obj["工资"] = item.sjshaochuDouble || "";
        obj[this.jcpeizhi.userBieming || "求职者"] = item.userName || "";

        // 判断简历附件是否上传
        obj["简历"] = (item.sjshaochuImgName != null && item.sjshaochuImgName !== "") ? "已上传" : "未上传";

        obj["电话"] = item.sjshaochuName || "";
        obj["学校"] = item.sjshaochuMark || "";
        obj["学历"] = item.sjshaochuMark1 || "";
        obj["专业"] = item.sjshaochuMark2 || "";

        // 转换状态类型 (0申请, 1录取, 2拒绝)
        obj["状态"] = this.shenheType(item) || "";

        return obj;
      });

      // 4. 将 JSON 数据转换为 Excel 工作表
      const filename = "求职信息导出.xlsx";
