  ...
  <section style="margin: 10px">
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px; margin-left: 10px" >
      <el-form :inline="true" :model="filters">
        <el-form-item>
          <el-input v-model="filters.yxinxiName" size="small" placeholder="通知" ></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="info" size="small" @click="query">查询</el-button>
        </el-form-item>
        <el-form-item>
          <el-button type="danger" size="small" @click="deleteYxinxis" :disabled="selectedIds.length === 0">批量删除</el-button>
        </el-form-item>
    ...
    <el-table ref="exportTableRef" @selection-change="handleSelectionChange" id="myTable" :data="datalist" border stripe v-loading="listLoading" highlight-current-row max-height="800" size="small" style="width: 100%">
      ...
      <el-table-column sortable prop="yxinxiId" align="center" label="Id" min-width="20%" v-if="false"></el-table-column>
      <el-table-column prop="yonghuId" align="center" :label=jcpeizhi.yonghuBieming min-width="20%" v-if="false"></el-table-column>
      <el-table-column prop="yonghuName" align="center" :label=jcpeizhi.yonghuBieming min-width="20%"></el-table-column>
      <el-table-column prop="yxinxiName" align="center" label="通知" :show-overflow-tooltip="true" min-width="40%"></el-table-column>
      ...
      <el-table-column prop="yxinxiMark" align="center" label="回复" :show-overflow-tooltip="true" min-width="40%"></el-table-column>
      <el-table-column sortable prop="yxinxiDate" align="center" label="时间" width="130" :formatter="formatDate" v-if="false"></el-table-column>
      <!--操作按钮开始-->
      <el-table-column label="操作" align="center" width="100">
        <template #default="scope">
          <el-button v-if="role == 'yonghu'" type="success" size="mini" @click="handleEdit(scope.$index, scope.row)" >编辑</el-button>
          <el-button v-if="role == 'user'" type="success" size="mini" @click="danduChuli(scope.$index, scope.row)" >回复</el-button>
        </template>
      </el-table-column>
      <!--操作按钮结束-->
    </el-table>
    ...
    <!--表单-->
    <el-dialog :title=jcpeizhi.yxinxiBieming v-model="formVisible" width="40%" :close-on-click-modal="false">
    ...
    <el-dialog title="上传信息" v-model="formShangchuan" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="附件" prop="yxinxiImg">
          <el-input v-model="formData.yxinxiImg" readonly="true" style="width: 50%"></el-input>
    ...
    <el-dialog title="处理信息" v-model="formChuli" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="回复" prop="yxinxiMark">
          <el-input type="textarea" :rows="4" v-model="formData.yxinxiMark" style="width: 80%"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
    ...
    <el-dialog title="审核信息" v-model="formShenhe" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="状态" prop="yxinxiType">
          <el-select v-model="formData.yxinxiType" style="width: 50%" placeholder="请选择">
            <el-option label="通过" value="1"></el-option>
            <el-option label="拒绝" value="2"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="审核" prop="yxinxiMark3">
          <el-input type="textarea" :rows="4" v-model="formData.yxinxiMark3" style="width: 80%"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
    ...
    <el-dialog title="导入信息" v-model="formDaoru" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="excel表" prop="yxinxiImg">
          <el-input v-model="formData.yxinxiImg" readonly="true" style="width: 50%"></el-input>
          <el-button type="primary" size="small" style="margin-left: 15px" @click="showUpload">上传</el-button>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      selectedIds: [],//多选数据
      rules: {     
        yxinxiName: [{ required: true, message: "不能为空", trigger: "blur" }],
        yxinxiMark: [{ required: true, message: "不能为空", trigger: "blur" }],
        yxinxiMark1: [{ required: true, message: "不能为空", trigger: "blur" }],
        yxinxiMark2: [{ required: true, message: "不能为空", trigger: "blur" }],
        yxinxiDouble: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        yxinxiDouble1: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        yxinxiDouble2: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        yxinxiZong: [{ required: true, validator: isIntNum, trigger: "blur" }],
        yxinxiZong1: [{ required: true, validator: isIntNum, trigger: "blur" }],
        yxinxiZong2: [{ required: true, validator: isIntNum, trigger: "blur" }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    */
    ...
    handleSelectionChange(selection) {
      this.selectedIds = selection.map((item) => item.yxinxiId);
    ...
    getDatas() {
      ...
      let para = {
        yxinxiName: this.filters.yxinxiName,
      ...
      //console.log(para)
      this.listLoading = true;
      let url = base + "/getYxinxis?page=" + this.page.currentPage;
      //console.log(url);
      request.post(url, para).then((res) => {
        console.log(res);
    ...
    shangchuanSave() {
      this.$refs["formDataRef"].validate((valid) => {
        if (valid) {
          let url = base + "/addYxinxi?yxinxiId=" + this.formData.yxinxiId;
          this.btnLoading = true;
          request.post(url, this.formData).then((res) => {
            if (res.code == 200) {
              this.btnLoading = false;
    ...
    //添加修改
    save() {
      if (this.formData.yxinxiId != null) {
        //如果有ID就修改数据
        this.$refs["formDataRef"].validate((valid) => {
          if (valid) {
            let url = base + "/addYxinxi?yxinxiId=" + this.formData.yxinxiId;
    ...
    shezhiZhuangtai(index, row,zhuangtai) {
      this.listLoading = true;
      let para = {
        yxinxiType: zhuangtai,
        yxinxiId: row.yxinxiId,
      };
