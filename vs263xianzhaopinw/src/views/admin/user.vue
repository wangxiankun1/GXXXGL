  ...
  <section style="margin: 10px">
      ...
      <el-table-column label="操作" align="center" width="180">
        <template #default="scope">
          <el-button
              ...
              size="mini"
              type="success"
              @click="danduChuli(scope.$index, scope.row)">
            编辑
          </el-button>
    ...
    <el-dialog title="修改信息" v-model="formChuli" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" :rules="rules" label-width="100px">      
        ...
        <el-form-item label="简历" prop="userImg">
          <el-input v-model="formData.userImg" readonly="true" style="width: 50%"></el-input>
          <el-button type="primary" size="small" style="margin-left: 15px" @click="showUpload">上传</el-button>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    */
    ...
    exportTable() {
      ...
      const selectedData = this.datalist.filter(item => this.selectedIds.includes(item.userId));

      // 3. 提取当前模块需要的字段并转换为中文表头，包含序号、性别判断及简历状态
      const exportJson = selectedData.map((item, index) => {
        let obj = {};
        obj["序号"] = index + 1;
        obj["用户名"] = item.userName || "";
        obj["姓名"] = item.userXingming || "";
        // 判断性别：0为男，1为女
        obj["性别"] = item.userSex == 0 ? "男" : (item.userSex == 1 ? "女" : "");
        obj["年龄"] = item.userAge || "";
        obj["电话"] = item.userPhone || "";
        obj["学校"] = item.userMark || "";
        obj["学历"] = item.userMark1 || "";
        obj["专业"] = item.userMark2 || "";

        // 简历附件列判断
        obj["简历"] = (item.userImgName != null && item.userImgName !== "") ? "已上传" : "未上传";

        return obj;
      });

      // 4. 将 JSON 数据转换为 Excel 工作表
      const filename = "用户信息导出.xlsx";
    ...
    resetPassword(index, row) {
      this.$confirm("确认将该用户的密码重置为 123456 吗?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
          .then(() => {
            this.listLoading = true;
            // 复用后端的修改接口 addUser
            let url = base + "/addUser?userId=" + row.userId;
            let para = {
              userId: row.userId,
              userPassword: "123456" // 默认重置的密码设为 123456
            };
            request.post(url, para).then((res) => {
              this.listLoading = false;
              if (res.code == 200) {
