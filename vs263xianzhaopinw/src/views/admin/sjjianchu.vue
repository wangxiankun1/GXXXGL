  ...
  <section style="margin: 10px">
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px; margin-left: 10px" >
      <el-form :inline="true" :model="filters">
        <el-form-item>
          <el-input v-model="filters.sjjianchuName" size="small" placeholder="名称" ></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="info" size="small" @click="query">查询</el-button>
        </el-form-item>
        <el-form-item>
          <el-button type="danger" size="small" @click="deleteSjjianchus" :disabled="selectedIds.length === 0">批量删除</el-button>
        </el-form-item>
    ...
    <el-table ref="exportTableRef" @selection-change="handleSelectionChange" id="myTable" :data="datalist" border stripe v-loading="listLoading" highlight-current-row max-height="800" size="small" style="width: 100%">
      ...
      <el-table-column sortable prop="sjjianchuId" align="center" label="Id" min-width="20%" v-if="false"></el-table-column>
      ...
      <el-table-column prop="sjjianchuName" align="center" label="咨询" :show-overflow-tooltip="true" min-width="80%"></el-table-column>
      ...
      <el-table-column prop="sjjianchuMark" align="center" label="回复" :show-overflow-tooltip="true" min-width="80%"></el-table-column>
      <el-table-column sortable prop="sjjianchuDate" align="center" label="时间" width="130" :formatter="formatDate" v-if="false"></el-table-column>
      <!--操作按钮开始-->
    ...
    <!--表单-->
    <el-dialog :title=jcpeizhi.sjjianchuBieming v-model="formVisible" width="40%" :close-on-click-modal="false">
    ...
    <el-dialog title="上传信息" v-model="formShangchuan" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="附件" prop="sjjianchuImg">
          <el-input v-model="formData.sjjianchuImg" readonly="true" style="width: 50%"></el-input>
    ...
    <el-dialog title="处理信息" v-model="formChuli" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="回复" prop="sjjianchuMark">
          <el-input type="textarea" :rows="4" v-model="formData.sjjianchuMark" style="width: 80%"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
    ...
    <el-dialog title="审核信息" v-model="formShenhe" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="状态" prop="sjjianchuType">
          <el-select v-model="formData.sjjianchuType" style="width: 50%" placeholder="请选择">
            <el-option label="通过" value="1"></el-option>
            <el-option label="拒绝" value="2"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="审核" prop="sjjianchuMark3">
          <el-input type="textarea" :rows="4" v-model="formData.sjjianchuMark3" style="width: 80%"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
    ...
    <el-dialog title="导入信息" v-model="formDaoru" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="excel表" prop="sjjianchuImg">
          <el-input v-model="formData.sjjianchuImg" readonly="true" style="width: 50%"></el-input>
          <el-button type="primary" size="small" style="margin-left: 15px" @click="showUpload">上传</el-button>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      selectedIds: [],//多选数据
      rules: {     
        sjjianchuName: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjjianchuMark: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjjianchuMark1: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjjianchuMark2: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjjianchuMark3: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjjianchuDouble: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        sjjianchuDouble1: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        sjjianchuDouble2: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        sjjianchuZong: [{ required: true, validator: isIntNum, trigger: "blur" }],
        sjjianchuZong1: [{ required: true, validator: isIntNum, trigger: "blur" }],
        sjjianchuZong2: [{ required: true, validator: isIntNum, trigger: "blur" }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    //富文本
    editorChange(info) {
      this.formData.sjjianchuMark = info;
    },
    //打开修改界面
    ...
    handleDelete(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          ...
          let para = {
            delIds: row.sjjianchuId,
          };
          let url = base + "/deleteSjjianchu";
    ...
    handleConfirm() {
      ...
      request.post(url, formData).then((res) => {
        ...
        console.log(furl + "  ------------");
        this.formData.sjjianchuImg = furl;
        this.hideUpload();
        console.log(res);
      });
    },
    ...
    */
    ...
    handleSelectionChange(selection) {
      this.selectedIds = selection.map((item) => item.sjjianchuId);
    ...
    getDatas() {
      ...
      let para = {
        sjjianchuName: this.filters.sjjianchuName,
      ...
      //console.log(para)
      this.listLoading = true;
      let url = base + "/getSjjianchus?page=" + this.page.currentPage;
      //console.log(url);
      request.post(url, para).then((res) => {
        console.log(res);
    ...
    shangchuanSave() {
      this.$refs["formDataRef"].validate((valid) => {
        if (valid) {
          let url = base + "/addSjjianchu?sjjianchuId=" + this.formData.sjjianchuId;
          this.btnLoading = true;
          request.post(url, this.formData).then((res) => {
            if (res.code == 200) {
              this.btnLoading = false;
    ...
    //添加修改
    save() {
      if (this.formData.sjjianchuId != null) {
        //如果有ID就修改数据
        this.$refs["formDataRef"].validate((valid) => {
          if (valid) {
            let url = base + "/addSjjianchu?sjjianchuId=" + this.formData.sjjianchuId;
    ...
    //根据数据状态显示文本
    zhuangtaiType(row){
        if(row.sjjianchuType==0){
             return "启用";
        }
        if(row.sjjianchuType==1){
             return "禁用";
        }
    },
    //根据数据状态显示文本
    ...
    shezhiZhuangtai(index, row,zhuangtai) {
      this.listLoading = true;
      let para = {
        sjjianchuType: zhuangtai,
        sjjianchuId: row.sjjianchuId,
      };
