  ...
  <section style="margin: 10px">
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px; margin-left: 10px" >
      <el-form :inline="true" :model="filters">
        <el-form-item>
          <el-input v-model="filters.sjleixingName" size="small" placeholder="名称" ></el-input>
        </el-form-item>
        <el-form-item>
        ...
        <el-form-item v-if="role == 'admin' || role == 'admin1'">
          <el-button type="danger" size="small" @click="deleteSjleixings" :disabled="selectedIds.length === 0">批量删除</el-button>
        </el-form-item>
    ...
    <el-table ref="exportTableRef" @selection-change="handleSelectionChange" id="myTable" :data="datalist" border stripe v-loading="listLoading" highlight-current-row max-height="800" size="small" style="width: 100%">
      ...
      <el-table-column sortable prop="sjleixingId" align="center" label="Id" min-width="20%" v-if="false"></el-table-column>
      <el-table-column sortable prop="sjleixingName" align="center" label="名称" min-width="40%"></el-table-column>
      <el-table-column prop="sjleixingMark" align="center" label="详情" :show-overflow-tooltip="true" min-width="80%"></el-table-column>
      <el-table-column prop="sjleixingDate" align="center" label="时间" width="140" :formatter="formatDate" v-if="false"></el-table-column>
      <!--操作按钮开始-->
    ...
    <!--表单-->
    <el-dialog :title=jcpeizhi.sjleixingBieming v-model="formVisible" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" :rules="rules" label-width="100px">
        <el-form-item label="名称" prop="sjleixingName">
          <el-input v-model="formData.sjleixingName" style="width: 50%"></el-input>
        </el-form-item>
        <el-form-item label="详情" prop="sjleixingMark">
          <el-input type="textarea" :rows="4" v-model="formData.sjleixingMark" style="width: 80%"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog title="上传信息" v-model="formShangchuan" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="附件" prop="sjleixingImg">
          <el-input v-model="formData.sjleixingImg" readonly="true" style="width: 50%"></el-input>
    ...
    <el-dialog title="导入信息" v-model="formDaoru" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="excel表" prop="sjleixingImg">
          <el-input v-model="formData.sjleixingImg" readonly="true" style="width: 50%"></el-input>
          <el-button type="primary" size="small" style="margin-left: 15px" @click="showUpload">上传</el-button>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      selectedIds: [],//多选数据
      rules: {     
        sjleixingName: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjleixingMark: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjleixingMark1: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjleixingMark2: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjleixingDizhi: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjleixingDouble: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        sjleixingDouble1: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        sjleixingType: [{ required: true, validator: isIntNum, trigger: "blur" }],
        sjleixingType1: [{ required: true, validator: isIntNum, trigger: "blur" }],
        sjleixingPhone: [{ required: true, validator: isPhone, trigger: "blur" }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    //富文本
    editorChange(info) {
      this.formData.sjleixingMark = info;
    },
    //打开修改界面
    ...
    handleDelete(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          ...
          let para = {
            delIds: row.sjleixingId,
          };
          let url = base + "/deleteSjleixing";
    ...
    handleConfirm() {
      ...
      request.post(url, formData).then((res) => {
        ...
        console.log(furl + "  ------------");
        this.formData.sjleixingImg = furl;
        this.hideUpload();
        console.log(res);
      });
    },
    ...
    */
    ...
    handleSelectionChange(selection) {
      this.selectedIds = selection.map((item) => item.sjleixingId);
    ...
    getDatas() {
      ...
      let para = {
        sjleixingName: this.filters.sjleixingName,
        sjleixingPhone: this.filters.sjleixingPhone,
      ...
      //console.log(para)
      this.listLoading = true;
      let url = base + "/getSjleixings?page=" + this.page.currentPage;
      //console.log(url);
      request.post(url, para).then((res) => {
        console.log(res);
    ...
    shangchuanSave() {
      this.$refs["formDataRef"].validate((valid) => {
        if (valid) {
          let url = base + "/addSjleixing?sjleixingId=" + this.formData.sjleixingId;
          this.btnLoading = true;
          request.post(url, this.formData).then((res) => {
            if (res.code == 200) {
              this.btnLoading = false;
    ...
    //添加修改
    save() {
      if (this.formData.sjleixingId != null) {
        //如果有ID就修改数据
        this.$refs["formDataRef"].validate((valid) => {
          if (valid) {
            let url = base + "/addSjleixing?sjleixingId=" + this.formData.sjleixingId;
    ...
    //根据数据状态显示文本
    zhuangtaiType(row){
        if(row.sjleixingType==0){
             return "启用";
        }
        if(row.sjleixingType==1){
             return "禁用";
        }
    },
    //单项更改状态
    shezhiZhuangtai(index, row,zhuangtai) {
      this.listLoading = true;
      let para = {
        sjleixingType: zhuangtai,
        sjleixingId: row.sjleixingId,
      };
