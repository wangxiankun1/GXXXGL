  ...
  <section style="margin: 10px">
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px; margin-left: 10px" >
      <el-form :inline="true" :model="filters">
        <el-form-item>
          <el-input v-model="filters.sjpinglunName" size="small" placeholder="名称" ></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="info" size="small" @click="query">查询</el-button>
        </el-form-item>
        <el-form-item>
          <el-button type="danger" size="small" @click="deleteSjpingluns" :disabled="selectedIds.length === 0">批量删除</el-button>
        </el-form-item>
    ...
    <el-table ref="exportTableRef" @selection-change="handleSelectionChange" id="myTable" :data="datalist" border stripe v-loading="listLoading" highlight-current-row max-height="800" size="small" style="width: 100%">
      ...
      <el-table-column sortable prop="sjpinglunId" align="center" label="Id" min-width="20%" v-if="false"></el-table-column>
      <el-table-column prop="userId" align="center" :label=jcpeizhi.userBieming min-width="20%" v-if="false"></el-table-column>
      <el-table-column prop="userName" align="center" :label=jcpeizhi.userBieming min-width="40%"></el-table-column>
      ...
      <el-table-column prop="sjpinglunName" align="center" label="评论" :show-overflow-tooltip="true" min-width="80%"></el-table-column>
      <el-table-column prop="yonghuId" align="center" :label=jcpeizhi.yonghuBieming min-width="20%" v-if="false"></el-table-column>
      <el-table-column prop="yonghuName" align="center" :label=jcpeizhi.yonghuBieming min-width="40%"></el-table-column>
      <el-table-column prop="sjpinglunMark" align="center" label="回复" :show-overflow-tooltip="true" min-width="80%"></el-table-column>
      <el-table-column sortable prop="sjpinglunDate" align="center" label="时间" width="130" :formatter="formatDate" v-if="false"></el-table-column>
      <!--操作按钮开始-->
      <el-table-column label="操作" align="center" width="150">
        <template #default="scope">
          <el-button v-if="role == 'user'" type="success" size="mini" @click="handleEdit(scope.$index, scope.row)" >编辑</el-button>
          <el-button v-if="role == 'yonghu'" type="warning" size="mini" @click="danduChuli(scope.$index, scope.row)" >回复</el-button>
    ...
    <!--表单-->
    <el-dialog :title=jcpeizhi.sjpinglunBieming v-model="formVisible" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" :rules="rules" label-width="100px">
        <el-form-item label="评论" prop="sjpinglunName">
          <el-input type="textarea" :rows="4" v-model="formData.sjpinglunName" style="width: 80%"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog title="上传信息" v-model="formShangchuan" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="附件" prop="sjpinglunImg">
          <el-input v-model="formData.sjpinglunImg" readonly="true" style="width: 50%"></el-input>
    ...
    <el-dialog title="处理信息" v-model="formChuli" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="回复" prop="sjpinglunMark">
          <el-input type="textarea" :rows="4" v-model="formData.sjpinglunMark" style="width: 80%"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
    ...
    <el-dialog title="审核信息" v-model="formShenhe" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="状态" prop="sjpinglunType">
          <el-select v-model="formData.sjpinglunType" style="width: 50%" placeholder="请选择">
            <el-option label="通过" value="1"></el-option>
            <el-option label="拒绝" value="2"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="审核" prop="sjpinglunMark3">
          <el-input type="textarea" :rows="4" v-model="formData.sjpinglunMark3" style="width: 80%"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
    ...
    <el-dialog title="导入信息" v-model="formDaoru" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="excel表" prop="sjpinglunImg">
          <el-input v-model="formData.sjpinglunImg" readonly="true" style="width: 50%"></el-input>
          <el-button type="primary" size="small" style="margin-left: 15px" @click="showUpload">上传</el-button>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      selectedIds: [],//多选数据
      rules: {     
        sjpinglunName: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjpinglunMark: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjpinglunMark1: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjpinglunMark2: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjpinglunMark3: [{ required: true, message: "不能为空", trigger: "blur" }],
        sjpinglunDouble: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        sjpinglunDouble1: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        sjpinglunDouble2: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        sjpinglunZong: [{ required: true, validator: isIntNum, trigger: "blur" }],
        sjpinglunZong1: [{ required: true, validator: isIntNum, trigger: "blur" }],
        sjpinglunZong2: [{ required: true, validator: isIntNum, trigger: "blur" }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    //富文本
    editorChange(info) {
      this.formData.sjpinglunMark = info;
    },
    //打开修改界面
    ...
    handleDelete(index, row) {
      this.$confirm("确认删除该记录吗?", "提示", {
        ...
        .then(() => {
          ...
          let para = {
            delIds: row.sjpinglunId,
          };
          let url = base + "/deleteSjpinglun";
    ...
    handleConfirm() {
      ...
      request.post(url, formData).then((res) => {
        ...
        console.log(furl + "  ------------");
        this.formData.sjpinglunImg = furl;
        this.hideUpload();
        console.log(res);
      });
    },
    ...
    */
    ...
    handleSelectionChange(selection) {
      this.selectedIds = selection.map((item) => item.sjpinglunId);
    ...
    getDatas() {
      ...
      let para = {
        sjpinglunName: this.filters.sjpinglunName,
      ...
      //console.log(para)
      this.listLoading = true;
      let url = base + "/getSjpingluns?page=" + this.page.currentPage;
      //console.log(url);
      request.post(url, para).then((res) => {
        console.log(res);
    ...
    shangchuanSave() {
      this.$refs["formDataRef"].validate((valid) => {
        if (valid) {
          let url = base + "/addSjpinglun?sjpinglunId=" + this.formData.sjpinglunId;
          this.btnLoading = true;
          request.post(url, this.formData).then((res) => {
            if (res.code == 200) {
              this.btnLoading = false;
    ...
    //添加修改
    save() {
      if (this.formData.sjpinglunId != null) {
        //如果有ID就修改数据
        this.$refs["formDataRef"].validate((valid) => {
          if (valid) {
            let url = base + "/addSjpinglun?sjpinglunId=" + this.formData.sjpinglunId;
    ...
    //根据数据状态显示文本
    zhuangtaiType(row){
        if(row.sjpinglunType==0){
             return "启用";
        }
        if(row.sjpinglunType==1){
             return "禁用";
        }
    },
    //根据数据状态显示文本
    ...
    shezhiZhuangtai(index, row,zhuangtai) {
      this.listLoading = true;
      let para = {
        sjpinglunType: zhuangtai,
        sjpinglunId: row.sjpinglunId,
      };
