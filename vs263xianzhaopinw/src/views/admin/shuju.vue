  ...
  <section style="margin: 10px">
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px; margin-left: 10px" >
      <el-form :inline="true" :model="filters">
        <el-form-item>
          <el-input v-model="filters.shujuName" size="small" placeholder="名称" ></el-input>
        </el-form-item>
        <el-form-item :label=jcpeizhi.sjleixingBieming prop="sjleixingId">
          <el-select size="small" v-model="filters.sjleixingId" placeholder="请选择" style="width: 50%">
            <el-option label="请选择" value=""> </el-option>
            <el-option v-for="item in sjleixingList" :key="item.sjleixingId" :label="item.sjleixingName" :value="item.sjleixingId">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
        ...
        <el-form-item>
          <el-button type="primary" size="small" @click="handleAdd" v-if="role == 'yonghu'">新增</el-button>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" size="small" @click="exportTable" :disabled="selectedIds.length === 0">导出</el-button>
        </el-form-item>
        <el-form-item>
          <el-button type="danger" size="small" @click="deleteShujus" :disabled="selectedIds.length === 0">批量删除</el-button>
        </el-form-item>
    ...
    <el-table ref="exportTableRef" @selection-change="handleSelectionChange" id="myTable" :data="datalist" border stripe v-loading="listLoading" highlight-current-row max-height="800" size="small" style="width: 100%">
      ...
      <el-table-column sortable prop="shujuId" align="center" label="Id" min-width="20%" v-if="false"></el-table-column>
      <el-table-column prop="yonghuId" align="center" :label=jcpeizhi.yonghuBieming min-width="20%" v-if="false"></el-table-column>
      <el-table-column prop="yonghuName" align="center" :label=jcpeizhi.yonghuBieming min-width="20%" v-if="role == 'admin'"></el-table-column>
      <el-table-column prop="sjleixingId" align="center" :label=jcpeizhi.sjleixingBieming min-width="20%" v-if="false"></el-table-column>
      <el-table-column prop="sjleixingName" align="center" :label=jcpeizhi.sjleixingBieming min-width="40%"></el-table-column>
      <el-table-column sortable prop="shujuName" align="center" label="名称" min-width="40%"></el-table-column>
      <el-table-column prop="shujuMark1" align="center" label="学历" :show-overflow-tooltip="true" min-width="40%"></el-table-column>
      <el-table-column prop="shujuMark2" align="center" label="地点" :show-overflow-tooltip="true" min-width="40%"></el-table-column>
      <el-table-column prop="shujuMark3" align="center" label="要求" :show-overflow-tooltip="true" min-width="80%"></el-table-column>
      ...
      <!--操作按钮开始-->
      <el-table-column label="操作" align="center" width="150" v-if="role == 'yonghu'">
        <template #default="scope">
    ...
    <!--表单-->
    <el-dialog :title=jcpeizhi.shujuBieming v-model="formVisible" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" :rules="rules" label-width="100px">
        <el-form-item label="名称" prop="shujuName">
          <el-input v-model="formData.shujuName" style="width: 50%"></el-input>
        </el-form-item>
        <el-form-item label="学历" prop="shujuMark1">
          <el-input v-model="formData.shujuMark1" style="width: 50%"></el-input>
        </el-form-item>
        <el-form-item label="地点" prop="shujuMark2">
          <el-input v-model="formData.shujuMark2" style="width: 50%"></el-input>
        </el-form-item>
        <el-form-item label="要求" prop="shujuMark3">
          <el-input v-model="formData.shujuMark3" style="width: 50%"></el-input>
        </el-form-item>
        <el-form-item label="工资" prop="shujuDouble">
          <el-input v-model="formData.shujuDouble" style="width: 50%"></el-input>
        </el-form-item>
        <el-form-item label="人数" prop="shujuZong">
          <el-input v-model="formData.shujuZong" style="width: 50%"></el-input>
        </el-form-item>
        <el-form-item :label=jcpeizhi.sjleixingBieming prop="sjleixingId">
          <el-select v-model="formData.sjleixingId" placeholder="请选择" style="width: 50%">
            <el-option label="请选择" value="0"> </el-option>
            <el-option v-for="item in sjleixingList" :key="item.sjleixingId" :label="item.sjleixingName" :value="item.sjleixingId"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="图片" prop="shujuImg">
          <el-input v-model="formData.shujuImg" readonly="true" style="width: 50%"></el-input>
          <el-button type="primary" size="small" style="margin-left: 15px" @click="showUpload">上传</el-button>
        </el-form-item>
        <el-form-item>
          <WangEditor v-model="formData.shujuMark" :isClear="isClear" @change="editorChange"></WangEditor>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog title="上传信息" v-model="formShangchuan" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="附件" prop="shujuImg">
          <el-input v-model="formData.shujuImg" readonly="true" style="width: 50%"></el-input>
          <el-button type="primary" size="small" style="margin-left: 15px" @click="showUpload">上传</el-button>
        </el-form-item>
      </el-form>
    ...
    <el-dialog title="处理信息" v-model="formChuli" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        ...
        <el-form-item label="简介" prop="shujuMark1">
          <el-input type="textarea" :rows="4" v-model="formData.shujuMark1" style="width: 80%"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
    ...
    <el-dialog title="审核信息" v-model="formShenhe" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="状态" prop="shujuType">
          <el-select v-model="formData.shujuType" style="width: 50%" placeholder="请选择">
            <el-option label="通过" value="1"></el-option>
            <el-option label="驳回" value="2"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="审核" prop="shujuMark3">
          <el-input type="textarea" :rows="4" v-model="formData.shujuMark3" style="width: 80%"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
    ...
    <el-dialog title="导入信息" v-model="formDaoru" width="40%" :close-on-click-modal="false">
      <el-form :model="formData" ref="formDataRef" label-width="100px">      
        <el-form-item label="excel表" prop="shujuImg">
          <el-input v-model="formData.shujuImg" readonly="true" style="width: 50%"></el-input>
          <el-button type="primary" size="small" style="margin-left: 15px" @click="showUpload">上传</el-button>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      selectedIds: [],//多选数据
      rules: {     
        shujuName: [{ required: true, message: "不能为空", trigger: "blur" }],
        shujuMark: [{ required: true, message: "不能为空", trigger: "blur" }],
        shujuMark1: [{ required: true, message: "不能为空", trigger: "blur" }],
        shujuMark2: [{ required: true, message: "不能为空", trigger: "blur" }],
        shujuMark3: [{ required: true, message: "不能为空", trigger: "blur" }],
        shujuDouble: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        shujuDouble1: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        shujuDouble2: [{ required: true, validator: isDoubleNum, trigger: "blur" }],
        shujuZong: [{ required: true, validator: isIntNum, trigger: "blur" }],
        shujuZong1: [{ required: true, validator: isIntNum, trigger: "blur" }],
        shujuZong2: [{ required: true, validator: isIntNum, trigger: "blur" }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    */
    exportTable() {
      ...
      const selectedData = this.datalist.filter(item => this.selectedIds.includes(item.shujuId));

      // 3. 提取当前模块需要的字段并转换为中文表头，兼顾别名和序号
      const exportJson = selectedData.map((item, index) => {
        let obj = {};
        obj["序号"] = index + 1;
        obj[this.jcpeizhi.yonghuBieming || "企业"] = item.yonghuName || "";
        obj[this.jcpeizhi.sjleixingBieming || "类型"] = item.sjleixingName || "";
        obj["职位名称"] = item.shujuName || "";
        obj["学历要求"] = item.shujuMark1 || "";
        obj["工作地点"] = item.shujuMark2 || "";
        obj["具体要求"] = item.shujuMark3 || "";
        obj["招聘人数"] = item.shujuZong || "";
        obj["工资待遇"] = item.shujuDouble || "";

        return obj;
      });

      // 4. 将 JSON 数据转换为 Excel 工作表
      const filename = "职位信息导出.xlsx";
    ...
    handleSelectionChange(selection) {
      this.selectedIds = selection.map((item) => item.shujuId);
    ...
    getDatas() {
      ...
      //console.log(para)
      this.listLoading = true;
      let url = base + "/getShujus?page=" + this.page.currentPage;
      //console.log(url);
      request.post(url, para).then((res) => {
        console.log(res);
    ...
    shangchuanSave() {
      this.$refs["formDataRef"].validate((valid) => {
        if (valid) {
          let url = base + "/addShuju?shujuId=" + this.formData.shujuId;
          this.btnLoading = true;
          request.post(url, this.formData).then((res) => {
            if (res.code == 200) {
              this.btnLoading = false;
