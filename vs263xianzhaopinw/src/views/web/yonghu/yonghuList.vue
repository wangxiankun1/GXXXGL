  ...
  <div class="product-page">
    <div class="container">
      ...
      <section class="product-list">
        <h2>{{ webJcpeizhi.yonghuBieming }}信息</h2>
        <div class="product-grid">
          <div
            class="product-card"
            v-for="yonghu in yonghuList" :key="yonghu.yonghuId"
          >
            <a :href="'/yonghuShow?yonghuId=' + yonghu.yonghuId">
              <img :src="yonghu.yonghuImg" />
              <h3>{{ yonghu.yonghuMark1 }}</h3>
            </a>
          </div>
        </div>
        <div class="pagebox">
          <pagination :num="num" :limit="limit" @getList="getYonghuList" />
        </div>
      </section>
      <!-- 左侧：分类和搜索 -->
      <aside class="sidebar">
        ...
        <div class="search-box">
          <input
            type="text"
            :placeholder="webJcpeizhi.yonghuBieming + '名称'"
            v-model="formData.yonghuMark1"
            @keyup.enter="query"
          />
          <button @click="query">
            <img
...
export default {
  ...
  watch: {
    '$route.query': {
      immediate: true,
      handler(newQuery) {
        this.yonghuMark1 = newQuery.yonghuMark1 || '';
        this.formData.yonghuMark1 = this.yonghuMark1;
        this.buzhiId = newQuery.buzhiId || '';
        this.yonghuId = newQuery.yonghuId || '';
        this.getYonghuList(0);
      }
    }
  },
  methods: {
    getYonghuList(value) {
      ...
      const para = {
        yonghuMark1: this.yonghuMark1,
        buzhiId: this.buzhiId,
        yonghuId: this.yonghuId,
      };
      const url = `${base}/getYonghus?page=${this.currentPage}&rows=${this.limit}`;
    ...
    getBuzhiList() {
      request.post(`${base}/getBuzhis`, {}).then((res) => {
        if (res.code === 200) {
    ...
    query() {
      const name = this.formData.yonghuMark1;
      const queryObj = {};
      if (name) queryObj.yonghuMark1 = name;
      if (this.buzhiId) queryObj.buzhiId = this.buzhiId;
      this.$router.push({ path: "/yonghuList", query: queryObj });
    },
    goToLeixing(buzhiId) {
      const queryObj = { buzhiId };
      if (this.formData.yonghuMark1) queryObj.yonghuMark1 = this.formData.yonghuMark1;
      this.$router.push({ path: "/yonghuList", query: queryObj });
    },
  },
};
