...
export default {
  ...
  created() {
    this.shujuName = this.$route.query.shujuName;
    this.sjleixingId = this.$route.query.sjleixingId;
    this.yonghuId = this.$route.query.yonghuId;
    this.getJcpeizhi();
    this.getSjleixingList();
    // 不在 created 中加载 getShujuList，转由 watch 控制
  },
  watch: {
    '$route.query.sjleixingId': {
      immediate: true,
      handler(newVal) {
        this.sjleixingId = newVal;
        this.getShujuList(0);
      }
    },
    '$route.query.shujuName': {
      immediate: true,
      handler(newVal) {
        this.shujuName = newVal;
        this.getShujuList(0);
      }
    }
  },
  methods: {
    getShujuList(value) {
      ...
      const url = `${base}/tuijianShujus?page=${this.currentPage}&rows=${this.limit}`;
    ...
    query() {
      ...
      if (name) {
        this.$router.push(`/shujuList?shujuName=${name}`);
      } else {
